...
export default function TablesPage() {
  ...
  const [showLocalResSearchResults, setShowLocalResSearchResults] =
    useState(false);
  ...
  const currentFloorTablesQueryResult =
    TablesService.useTablesByFloor(selectedFloorId);
  const currentFloorTables = currentFloorTablesQueryResult.data || [];

  const isLoading =
    floorsQueryResult.isLoading ||
  ...
  const handleCreateTable = (e) => {
    ...
    if (
      allTables.some(
        (t) => t.id.toLowerCase() === newTableId.trim().toLowerCase(),
      )
    ) {
      alert(
        `Table ID "${newTableId.trim()}" already exists. Please choose a unique ID.`,
      );
      return;
    }
  ...
  const handleDeleteFloor = () => {
    if (
      !window.confirm(
        `Are you sure you want to delete the floor "${activeFloor?.name || ""}" and all its tables? This action cannot be undone.`,
      )
    ) {
      return;
    }
  ...
  return (
    <>
        ...
        <section className={styles.contentSection}>
          <div className={styles.cardWrapper}>
            ...
            <div className={styles.controlsRow}>
              <div className={styles.btnGroup}>
                <button
                  className={
                    activeTab === "MAP" ? styles.viewBtnActive : styles.viewBtn
                  }
                ...
                <button
                  className={
                    activeTab === "LIST" ? styles.viewBtnActive : styles.viewBtn
                  }
                  onClick={() => {
                    setActiveTab("LIST");
            ...
            {activeTab === "MAP" && (
              <>
                ...
                <div className={canvasStyles.floorSwitcherRow}>
                      ...
                      <button
                        ...
                        className={
                          selectedFloorId === floor.id
                            ? canvasStyles.floorTabActive
                            : canvasStyles.floorTab
                        }
                        onClick={() => {
                  ...
                  <div className={canvasStyles.actionToolbar}>
                    ...
                    <button
                      className={
                        isEditMode
                          ? canvasStyles.editToggleBtnActive
                          : canvasStyles.editToggleBtn
                      }
                ...
                <div className={styles.floorMapScrollWrapper}>
                  <div
                    className={`${styles.floorMapCard} ${styles.mapCardCanvasOverride}`}
                  >
                    <FloorCanvas
                      floorId={selectedFloorId}
                          ...
                          >
                            {t.status === "reserved"
                              ...
                              : t.status === "occupied"
                                ? t.currentCustomerId
                                  ? customers.find(
                                      (c) => c.id === t.currentCustomerId,
                                    )?.name || "Seated Customer"
                                  : "Walk-in"
                                : "—"}
                          </td>
                        ...
                        return (
                          <div key={t.id} className={styles.tableRowGantt}>
                            ...
                            <div className={styles.tableMetaCol}>
                              <span className={styles.tableNameGantt}>
                                {t.id}
                              </span>
                              <span className={styles.tableDescGantt}>
                            ...
                            <div className={styles.timeGridCol}>
                              ...
                              <div
                                ...
                                onClick={(e) => {
                                  ...
                                  const hourClicked =
                                    17 + Math.floor(totalMins / 60);
                                  const minClicked =
                                    Math.floor((totalMins % 60) / 15) * 15; // snap to nearest 15 mins
                              ...
                              {tableRes.map((res) => {
                                ...
                                if (res.status === "SEATED")
                                  pillClass = styles.pillSeated;
                                else if (res.status === "CONFIRMED")
                                  pillClass = styles.pillConfirmed;
                                else if (res.status === "PENDING")
                                  pillClass = styles.pillPending;
                                ...
                                return (
                                  ...
                                  >
                                    <span className={styles.pillGuest}>
                                      {res.guest}
                                    </span>
                                    <span
                                      className={styles.pillMeta}
                                      suppressHydrationWarning
                                    >
                                      {res.details.split("•")[0]} •{" "}
                      ...
                      return (
                        <div key={res.id} className={styles.unassignedCard}>
                          <div className={styles.unassignedCardHeader}>
                            <span className={styles.unassignedGuest}>
                              {res.guest}
                            </span>
                            <span
                              className={`${styles.statusBadge} ${badgeClass} ${styles.unassignedBadgeSmall}`}
                            >
                              {res.status}
                            </span>
                          </div>
                          <p className={styles.unassignedDetails}>
                            {res.details}
                          </p>

                          <div className={styles.unassignedStatusRow}>
                            <span
                              className={styles.unassignedTime}
                              suppressHydrationWarning
                            >
                              {formatTimeTo12Hour(res.time)}
                            </span>
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                ReservationService.assignTableToReservation(
                                  res.id,
                                  tableId,
                                );
                              }}
          ...
          <div className={styles.darkBand}>
            <div className={styles.darkBandTimeline}>
              <h3 className={styles.darkBandLabel}>OCCUPANCY TIMELINE</h3>
            ...
            <div className={styles.darkBandStats}>
              <div>
                <div className={styles.statLabel}>AVG TURN</div>
      ...
      >
        ...
        >
          <div className={modalStyles.formGroup}>
            <label className={modalStyles.label} htmlFor="floor-name">
              Floor Name
            </label>
            <input
              type="text"
      ...
      >
        ...
        >
          <div className={modalStyles.formGroup}>
            <label className={modalStyles.label} htmlFor="table-zone">
              Zone
            </label>
            <select
              id="table-zone"
          ...
          <div className={modalStyles.formGroup}>
            <label className={modalStyles.label} htmlFor="table-id">
              Table ID
            </label>
            <input
              type="text"
          ...
          {newTableZone !== "BAR" && (
            <div className={modalStyles.formGroup}>
              <label className={modalStyles.label} htmlFor="table-shape">
                Shape
              </label>
              <select
                id="table-shape"
          ...
          <div className={modalStyles.formGroup}>
            <label className={modalStyles.label} htmlFor="table-seats">
              Capacity (Seats)
            </label>
            <input
              type="number"
      ...
      >
        <form
          onSubmit={handleCreateLocalReservation}
          className={modalStyles.form}
        >
          <div className={modalStyles.formGroup}>
            <label className={modalStyles.label} htmlFor="local-guest">
              Guest Name
            </label>
            {localResCustomerId ? (
              <div className={styles.linkedBadge}>
                <div className={styles.linkedInfo}>
                  ...
                  <span className={styles.linkedGuestMeta}>
                    {customers.find((c) => c.id === localResCustomerId)?.phone}{" "}
                    • {customers.find((c) => c.id === localResCustomerId)?.tier}{" "}
                    Segment
                  </span>
                </div>
                <button
                      ...
                      {customers
                        .filter(
                          (c) =>
                            c.name
                              .toLowerCase()
                              .includes(localResSearchQuery.toLowerCase()) ||
                        ...
                        .map((c) => (
                          ...
                          >
                            <div>
                              ...
                              <div className={styles.searchResultPhone}>
                                {c.phone}
                              </div>
                            </div>
                            <span
                              className={`${styles.searchResultTier} ${
                                c.tier === "Platinum"
                                  ? styles.tierPlatinum
                                  : c.tier === "VIP"
                                    ? styles.tierVip
          ...
          <div className={modalStyles.formGroup}>
            <label className={modalStyles.label} htmlFor="local-details">
              Party Details / Notes
            </label>
            <input
              type="text"
          ...
          <div className={modalStyles.formGroup}>
            ...
            <div className={styles.timeSelectorRow}>
              ...
              <select
                ...
                value={prefMinute}
                onChange={(e) =>
                  handlePrefilledTimeChange(prefHour, e.target.value, prefAmPm)
                }
              >
          ...
          <div className={modalStyles.formGroup}>
            <label className={modalStyles.label} htmlFor="local-status">
              Status
            </label>
            <select
              id="local-status"
          ...
          <div className={modalStyles.form}>
            <div className={styles.timelineDetailCard}>
              <div className={styles.timelineDetailRow}>
                ...
                <span className={styles.timelineDetailVal}>
                  {selectedRes.guest}
                </span>
              </div>
              <div className={styles.timelineDetailRow}>
                <span className={styles.timelineDetailLabel}>
                  Assigned Table
                </span>
                <span className={styles.timelineDetailVal}>
                  {selectedRes.tableId || "Unassigned"}
                </span>
              </div>
              <div className={styles.timelineDetailRow}>
                <span className={styles.timelineDetailLabel}>
              ...
              <div className={styles.timelineDetailRow}>
                ...
                <span
                  className={styles.timelineDetailTime}
                  suppressHydrationWarning
                >
                  {formatTimeTo12Hour(selectedRes.time)}
                </span>
              </div>
              <div className={styles.timelineDetailRow}>
                <span className={styles.timelineDetailLabel}>
              ...
              <div className={styles.modalActionRow}>
                <button
                  ...
                  onClick={() => {
                    ...
                    const nextIdx =
                      (statuses.indexOf(selectedRes.status) + 1) %
                      statuses.length;
      ...
      >
        <div className={modalStyles.form}>
          <div className={modalStyles.formGroup}>
            <label className={modalStyles.label} htmlFor="customer-search">
              Search CRM Customer
            </label>
            <input
              type="text"
          ...
          <div className={styles.walkInWrapper}>
            ...
            >
              ...
              <span className={styles.walkInSubtitle}>
                (No CRM account link)
              </span>
            </button>
          </div>
          ...
          <div className={modalStyles.formGroup}>
            ...
            <div className={styles.timeSelectorRow}>
              <select
                ...
                onChange={(e) =>
                  handleTimeChange(e.target.value, selectedMinute, selectedAmPm)
                }
              >
                {Array.from({ length: 12 }, (_, i) => i + 1).map((h) => (
                  <option key={h} value={h.toString().padStart(2, "0")}>
              ...
              <select
                ...
                onChange={(e) =>
                  handleTimeChange(selectedHour, e.target.value, selectedAmPm)
                }
              >
                {Array.from({ length: 60 }, (_, i) =>
                  i.toString().padStart(2, "0"),
